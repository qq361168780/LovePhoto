function lp_checked(user){
	if( user > 0){
		jQuery.post( lp.ajaxurl , {
			"action": "lp_ajax_checked",
			"user": user
		},function(){
			jQuery(".user-notify").remove();
...
function lp_liked(postid){
	if( postid > 0){
		jQuery("#like-" + postid).addClass("loading");
		jQuery.post( lp.ajaxurl , {
			"action": "lp_ajax_liked",
			"postid": postid
		},function(result){
			$("#like-" + postid).html(result);
			$(".single-like, .archive-like").on("mouseleave", ".liking", function(){
...
function lp_followed(author){
	if( author > 0){
		jQuery("#follow-button-" + author).addClass("loading");
		jQuery.post( lp.ajaxurl , {
			"action": "lp_ajax_followed",
			"authorid": author
		},function(result){
			jQuery("#follow-button-" + author).parent().parent().html(result);
		}).error(function(request){
			alert("Error：" + request.responseText);
			return false;
		})
		...
		calcPos: function() {
			...
			switch (this.opts.alignX) {
				...
				case 'inner-left':
					...
					break;
				case 'center':
					pos.l = xC - Math.floor(this.tipOuterW / 2) + 95;
...
;jQuery(document).ready(function($){
	...
	$("#content a.lp-user-profile").poshytip({
		className: 'tip-liquid',
		showTimeout: 1,
		alignTo: 'target',
                alignY: 'top',
                alignX: 'center',
		...
		content: function(updateCallback){
			...
			jQuery.get( lp.ajaxurl , {
				"action": "lp_ajax_profile",
				"user_id": uid
			},function(data){
				var container = data;
