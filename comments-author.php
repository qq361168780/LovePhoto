			...
			<form method="post" action="<?php echo home_url('/wp-comments-post.php'); ?>" id="comment_form">
				...
				<?php else : ?>
					<div class="comment-tips">
						请 <a class="notsignined" href="<?php bloginfo('url'); ?>/wp-login.php" class="bn-submit">登录</a> 后留言。 还没有帐号 <a href="<?php bloginfo('url'); ?>/wp-login.php?action=register" class="register-link">现在注册</a> 也可以使用 <a class="go-weibo" href="<?php echo plugins_url("wp-connect/login.php?go=sina");?>">新浪微博</a> 或 <a class="go-qq" href="<?php echo plugins_url("wp-connect/login.php?go=qzone");?>">QQ帐号</a> 直接登录。
					</div>				
				<?php endif; ?>
					...
					<div class="respond-footer clearfix">
						...
						<input id="submit" type="submit" name="submit" <?php if(!is_user_logged_in()) echo "disabled";?> value="<?php _e('留言'); ?>" class="submit" />
					</div>
